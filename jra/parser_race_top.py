...
class parser_race_top(prp.parser_post):
  def parse_content(self, soup):
        areas = soup.find_all("div", attrs = {"class":"kaisaiDayOutdiv"})

        kaisai_list = []
        kaisai_day_filter = re.compile("kaisaiDayLeftDiv?[0-9]")

        for area in areas:
            if area.find("table", attrs={"class": "jushoRaceList"}):
              continue

            day_tag = area.find("div", attrs = {"class":kaisai_day_filter})
            if day_tag :
                date = pu.func_parser.trim_clean(day_tag.get_text())
                date = re.sub(r'（.*）',"", date)
                #print(date)
                kaisai = {}
                kaisai['date'] = date
